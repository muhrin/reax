...
#     ],
# )
# def test_gradient_accumulation_scheduling_last_batch(
#     tmp_path, accumulate_grad_batches, limit_train_batches
# ):
...
#         pytest.param(
#             2, False, [f"epoch={i}.ckpt" for i in (2, 4)], id="CASE K=2 (2.5 epoch 4, 2.8 epoch 2)"
#         ),
#         pytest.param(
#             4, False, [f"epoch={i}.ckpt" for i in range(1, 5)], id="CASE K=4 (save all 4 base)"
#         ),
#         pytest.param(
#             3,
#             False,
...
# @pytest.mark.parametrize(
#     ("max_epochs", "max_updates", "is_done", "correct_trainer_epochs"),
#     [
#         (None, -1, False, None),
#         (-1, -1, False, -1),
#         (5, -1, False, 5),
...
#     max_epochs, max_updates, is_done, correct_trainer_epochs
# ):
#     trainer = reax.Trainer(demos.BoringModel()).fit(max_epochs=max_epochs, max_updates=max_updates)
#
#     assert trainer.max_epochs == correct_trainer_epochs
#     assert trainer.max_steps == max_updates
#
...
#             else pytest.warns(UserWarning, match="No checkpoint will be loaded")
#         )
#         with ctxt:
...
#             with pytest.warns(
#                 UserWarning, match="The best model of the previous `fit` call will be used"
#             ):
#                 trainer_fn(ckpt_path=ckpt_path)
#             assert trainer.ckpt_path == trainer.checkpoint_callback.best_model_path
#     else:
...
#     assert trainer.current_epoch == 0
#     assert (
#         not model.training_step_invoked
#     ), "`training_step` should not run when `limit_train_batches=0`"
#
...
#     trainer.strategy._launcher.launch = lambda function, *args, trainer, **kwargs: function(
#         *args, **kwargs
#     )
#
...
#
# @pytest.mark.parametrize(
#     "precision", ["32-true", pytest.param("16-mixed", marks=RunIf(min_cuda_gpus=1))]
# )
# @RunIf(sklearn=True)
...
#     with pytest.raises(
#         MisconfigurationException, match="`gradient_clip_algorithm` norm2 is invalid"
#     ):
...
#     with pytest.raises(
#         TypeError, match=r"__init__\(\) got an unexpected keyword argument 'abcdefg'"
#     ):
...
#     trainer = Trainer(
#         default_root_dir=tmp_path, max_epochs=1, enable_checkpointing=False, callbacks=[callback]
#     )
#
#     if stage == "fit":
#         trainer.fit(model)
...
#
# @pytest.mark.parametrize(
#     ("train_batches", "max_steps", "log_interval"), [(10, 10, 1), (3, 10, 1), (3, 10, 5)]
# )
...
#     trainer.fit(model)
#     expected_calls = [
#         call(metrics=ANY, step=s) for s in range(log_interval - 1, max_steps, log_interval)
#     ]
...
#                 )
#                 assert isinstance(
#                     trainer.predict_dataloaders[idx].batch_sampler, _IndexBatchSamplerWrapper
#                 )
...
#         if datamodule
#         else trainer.predict(model, dataloaders=dataloaders)
#     )
#
...
#     assert (
#         called_methods
#         == ["step", "training_step", "zero_grad", "backward"] * trainer.limit_train_batches
#     )
#
...
#     trainer.state.fn = TrainerFn.TESTING
#     with pytest.raises(
#         ValueError, match=r"with `fast_dev_run=True`. .* pass an exact checkpoint path"
#     ):
#         trainer._checkpoint_connector._parse_ckpt_path(
#             trainer.state.fn, ckpt_path="best", model_provided=False, model_connected=True
#         )
...
#     ), f"Could not find checkpoint at rank {trainer.global_rank}"
#     trainer.test()
#
#
...
#
#     trainer = Trainer(
#         default_root_dir=tmp_path, max_epochs=3, callbacks=[chk, CustomCallbackOnLoadCheckpoint()]
#     )
#     trainer.fit(model)
#
...
#     model = BoringModel()
#     trainer = Trainer(
#         default_root_dir=tmp_path, fast_dev_run=1, enable_checkpointing=False, logger=False
#     )
#
#     trainer.fit(model)
...
#
# @pytest.mark.parametrize(
#     "strategy", ["auto", pytest.param("ddp_spawn", marks=RunIf(skip_windows=True, mps=False))]
# )
...
#     trainer = Trainer(
#         default_root_dir=tmp_path, fast_dev_run=4, callbacks=[TestMetricRestartCallback()]
#     )
#     trainer.fit(model)
#     trainer.validate(model)
#     trainer.test(model)
...
#     [
#         pytest.param(
#             {"strategy": "auto"}, SingleDeviceStrategy, CPUAccelerator, 1, marks=RunIf(mps=False)
#         ),
...
#         ),
#         pytest.param(
#             {"strategy": DDPStrategy()}, DDPStrategy, CPUAccelerator, 1, marks=RunIf(mps=False)
#         ),
#         (
...
#     ],
# )
# def test_trainer_config_strategy(
#     monkeypatch, trainer_kwargs, strategy_cls, accelerator_cls, devices
# ):
...
#     with pytest.raises(
#         AttributeError, match="Saving a checkpoint is only possible if a model is attached"
#     ):
...
#     with (
#         mock.patch(
#             "lightning.pytorch.strategies.strategy.Strategy.on_exception"
#         ) as on_exception_mock,

...
# https://github.com/Lightning-AI/pytorch-lightning/blob/9177ec09caadcf88859e1f1e3e10a18e8832069a/tests/tests_pytorch/loggers/test_mlflow.py
#
...
from reax.loggers import mlflow
...
def test_mlflow_logger_exists(mlflow_mock, tmp_path):
    ...
    logger2 = mlflow.MlflowLogger("test", save_dir=str(tmp_path))
    ...
    logger3 = mlflow.MlflowLogger("new", save_dir=str(tmp_path))
...
def test_mlflow_run_name_setting(tmp_path):
    ...
    logger = mlflow.MlflowLogger(
        "test", run_name="run-name-1", tags={MLFLOW_RUN_NAME: "run-name-2"}, save_dir=str(tmp_path)
    )
    logger._mlflow_client = client = Mock()
    logger = mock_mlflow_run_creation(logger, experiment_id="exp-id")
    _ = logger.experiment
...
def test_mlflow_run_id_setting(mlflow_mock, tmp_path):
    ...
    logger = mlflow.MlflowLogger("test", run_id=run.info.run_id, save_dir=str(tmp_path))
    _ = logger.experiment
...
@mock.patch("reax.loggers.mlflow._get_resolve_tags", Mock())
@mock.patch("reax.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
...
def test_mlflow_logger_experiment_calls(mlflow_mock, tmp_path):
    ...
    logger = mlflow.MlflowLogger(
        "test", save_dir=str(tmp_path), artifact_location="my_artifact_location"
    )
...
def test_mlflow_logger_experiment_calls_with_synchronous(mlflow_mock, tmp_path, synchronous):
    ...
    mlflow_client.create_experiment.assert_called_once_with(
        name="test", artifact_location="my_artifact_location"
    )


@mock.patch("reax.loggers.mlflow._get_resolve_tags", Mock())
@mock.patch.dict("reax.loggers.mlflow.__dict__", {"_MLFLOW_SYNCHRONOUS_AVAILABLE": False})
...
def test_mlflow_logger_finalize(mlflow_mock, status, expected):
    logger = mlflow.MlflowLogger("test")
...
def test_mlflow_log_model(mlflow_mock, log_model, tmp_path):
    ...
    model = demos.BoringModel()
    logger = mlflow.MlflowLogger("test", save_dir=str(tmp_path), log_model=log_model)
...
def test_set_tracking_uri(mlflow_mock):
    ...
    logger = mlflow.MlflowLogger(tracking_uri="the_tracking_uri")

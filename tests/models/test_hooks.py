...
# https://github.com/Lightning-AI/pytorch-lightning/blob/9177ec09caadcf88859e1f1e3e10a18e8832069a/tests/tests_pytorch/models/test_hooks.py
#
...
class HookedDataModule(boring_classes.BoringDataModule):
    def __init__(self, called):
        ...
        for h in get_members(reax.DataModule):
            attr = getattr(self, h)
...
@pytest.mark.skip("overfit_batches not supported yet")
def test_on_train_epoch_end_metrics_collection(tmp_path):
    ...
    class CurrentModel(boring_classes.BoringModel):
        def training_step(self, *args, **kwargs):
            ...
            self.log_dict(
                {"step_metric": jnp.ndarray(-1), "shared_metric": 100}, logger=False, prog_bar=True
            )
            return output

        def on_train_epoch_end(self, stage):
            epoch = stage.epoch
            # both scalar tensors and Python numbers are accepted
            self.log_dict(
                {f"epoch_metric_{epoch}": jnp.ndarray(epoch), "shared_metric": 111},
                logger=False,
                prog_bar=True,
            )
    ...
    fit = trainer.fit(model, max_epochs=num_epochs, overfit_batches=2)
    assert fit.state.finished, f"Training failed with {fit.state}"
    metrics = trainer.progress_bar_listener.get_metrics(trainer, model)
...
class HookedListener(reax.TrainerListener):
    def __init__(self, called):
        def call(hook, fn, *args, **kwargs):
            out = fn(*args, **kwargs)
            d = {"name": f"Listener.{hook}"}
            if args:
                d["args"] = args
            if kwargs:
                d["kwargs"] = kwargs
        ...
        for h in get_members(reax.TrainerListener):
            attr = getattr(self, h)
            partial_h = partial(call, h, attr)
            update_wrapper(partial_h, attr)
            setattr(self, h, partial_h)
...
class HookedModel(boring_classes.BoringModel):
    def __init__(self, called):
        super().__init__()
        pl_module_hooks = get_members(reax.Module)
        # remove non-hooks
        ...
        module_hooks = {"parameters", "set_parameters", "rng_key"}
    ...
    def _auto_train_batch(
        trainer, model, batches, device, current_epoch=0, current_batch=0, **kwargs
    ):
            ...
            out.extend(
                [
                    ...
                    *(
                    ...
                    {
                        "name": "Listener.on_train_batch_end",
    ...
    def _eval_epoch(fn, trainer, model, batches, key, device):
        return [
            {"name": f"Listener.on_{fn}_epoch_start", "args": (trainer, model)},
            {"name": f"on_{fn}_epoch_start"},
            *HookedModel._eval_batch(fn, trainer, model, batches, key, device=device),
            {"name": f"Listener.on_{fn}_epoch_end", "args": (trainer, model)},
        ...
        for i in range(batches):
            out.extend(
                [
                    ...
                    {"name": "transfer_batch_to_device", "args": (ANY, device, 0)},
                    {"name": "on_after_batch_transfer", "args": (ANY, 0)},
                    {"name": f"Listener.on_{fn}_batch_start", "args": (trainer, model, ANY, i)},
                    {"name": f"on_{fn}_batch_start", "args": (ANY, i)},
                    {"name": "forward", "args": (ANY,)},
                    {"name": f"{fn}_step", "args": (ANY, i)},
                    {
                        "name": f"Listener.on_{fn}_batch_end",
    ...
    def _predict_batch(trainer, model, batches):
        ...
        for i in range(batches):
            out.extend(
                [
                    # TODO: {"name": "on_before_batch_transfer", "args": (ANY, 0)},
                    # TODO: {"name": "transfer_batch_to_device", "args": (ANY, device, 0)},
                    # TODO: {"name": "on_after_batch_transfer", "args": (ANY, 0)},
                    {"name": "on_stage_iter_starting"},
                    {"name": "on_predict_batch_start"},
                    {"name": "Listener.on_stage_iter_start"},
                    {"name": "Listener.on_predict_batch_start", "args": (trainer, model, ANY, i)},
                    {"name": "predict_step", "args": (ANY, i)},
                    {"name": "Listener.on_batch_end", "args": (ANY, i)},
                    {"name": "Listener.on_predict_batch_end", "args": (ANY,)},
                    {"name": "Listener.on_stage_iter_end"},
...
# def test_trainer_model_hook_system_fit(
#     override_on_validation_model_train, automatic_optimization, kwargs, tmp_path
# ):
#     called = []
#
...
#
#     callback = HookedListener(called)
#     train_batches = 2
#     val_batches = 2
#     trainer = Trainer(
#         default_root_dir=tmp_path,
...
#         {"name": "prepare_data"},
#         {"name": "Listener.setup", "args": (trainer, model), "kwargs": {"stage": "fit"}},
...
#         {"name": "Listener.on_fit_start", "args": (trainer, model)},
#         {"name": "on_fit_start"},
#         {"name": "zero_grad"},
#         {"name": "Listener.on_sanity_check_start", "args": (trainer, model)},
#         {"name": "val_dataloader"},
#         {"name": "train", "args": (False,)},
#         {"name": "on_validation_model_eval"},
#         {"name": "Listener.on_validation_start", "args": (trainer, model)},
#         {"name": "on_validation_start"},
#         *model._eval_epoch("validation", trainer, model, val_batches, "x", device=device),
#         {"name": "Listener.on_validation_end", "args": (trainer, model)},
#         {"name": "on_validation_end"},
#         *([{"name": "on_validation_model_train"}] if override_on_validation_model_train else []),
#         {"name": "Listener.on_sanity_check_end", "args": (trainer, model)},
#         {"name": "train_dataloader"},
#         {"name": "Listener.on_train_start", "args": (trainer, model)},
#         {"name": "on_train_start"},
#         {"name": "Listener.on_train_epoch_start", "args": (trainer, model)},
...
#         {"name": "on_validation_end"},
#         *([{"name": "on_validation_model_train"}] if override_on_validation_model_train else []),
#         {"name": "Listener.on_train_epoch_end", "args": (trainer, model)},
#         {
#             "name": "on_train_epoch_end"
#         },  # before ModelCheckpoint because it's a "monitoring callback"
#         # `ModelCheckpoint.save_checkpoint` is called here
#         {"name": "Listener.state_dict"},
#         {"name": "Listener.on_save_checkpoint", "args": (trainer, model, saved_ckpt)},
#         {"name": "on_save_checkpoint", "args": (saved_ckpt,)},
#         {"name": "Listener.on_train_end", "args": (trainer, model)},
#         {"name": "on_train_end"},
#         {"name": "Listener.on_fit_end", "args": (trainer, model)},
#         {"name": "on_fit_end"},
#         {"name": "Listener.teardown", "args": (trainer, model), "kwargs": {"stage": "fit"}},
...
#         {"name": "Listener.on_load_checkpoint", "args": (trainer, model, loaded_ckpt)},
#         {"name": "Listener.load_state_dict", "args": ({"foo": True},)},
#         {"name": "configure_optimizers"},
#         {"name": "Listener.on_fit_start", "args": (trainer, model)},
#         {"name": "on_fit_start"},
#         {"name": "zero_grad"},
...
#         {"name": "on_train_epoch_start"},
#         *model._train_batch(
#             trainer, model, 2, trainer.strategy.root_device, current_epoch=1, current_batch=0
#         ),
#         {"name": "Listener.on_train_epoch_end", "args": (trainer, model)},
#         {
#             "name": "on_train_epoch_end"
...
#         {"name": "on_train_start"},
#         *model._train_batch(
#             trainer, model, steps_after_reload, trainer.strategy.root_device, current_batch=1
#         ),
#         {"name": "Listener.on_train_epoch_end", "args": (trainer, model)},
#         {
#             "name": "on_train_epoch_end"
...
# def test_trainer_model_hook_system_eval(
#     tmp_path, override_on_x_model_train, batches, verb, noun, dataloader, key
# ):
#     called = []
#     model = HookedModel(called)
...
#         {"name": "configure_callbacks"},
#         {"name": "prepare_data"},
#         {"name": "Listener.setup", "args": (trainer, model), "kwargs": {"stage": verb}},
#         {"name": "setup", "kwargs": {"stage": verb}},
#         {"name": "configure_model"},
#         {"name": "zero_grad"},
#         *(hooks if batches else []),
#         {"name": "Listener.teardown", "args": (trainer, model), "kwargs": {"stage": verb}},
...
@pytest.mark.skip(reason="Not yet updated to work with REAX")
def test_trainer_model_hook_system_predict(tmp_path):
    called = []
    model = HookedModel(called)
    listener = HookedListener(called)
    batches = 2
    trainer = reax.Trainer(
        devices=1, default_root_dir=tmp_path, enable_progress_bar=False, listeners=[listener]
    )
    stage = trainer.predict(model, limit_batches=batches)
    expected = [
        {"name": "configure_listeners"},
        {"name": "on_stage_start", "args": (stage,)},
        {"name": "prepare_data"},
        {"name": "setup", "args": (stage,)},
        {"name": "predict_dataloader"},
        {"name": "configure_model", "args": (stage, ANY)},
        {"name": "on_predict_start", "args": (stage,)},
        {"name": "Listener.setup", "args": (trainer, stage)},
        # Stage start
        {"name": "Listener.on_stage_start", "args": (trainer, stage)},
        {"name": "Listener.on_predict_start", "args": (trainer, stage)},
        # Epoch start
        {"name": "on_predict_epoch_start", "args": (stage,)},
        {"name": "Listener.on_epoch_start"},
        {"name": "Listener.on_predict_epoch_start", "args": (trainer, model)},
        *model._predict_batch(trainer, model, batches),
        {"name": "Listener.on_epoch_end"},
        {"name": "Listener.on_predict_epoch_end", "args": (trainer, model)},
        {"name": "on_stage_end"},
        {"name": "on_predict_end"},
        {"name": "on_predict_epoch_end"},
        {"name": "Listener.on_stage_end"},
        {"name": "Listener.on_predict_end"},
        {"name": "Listener.teardown", "args": (trainer, stage)},
        {"name": "teardown", "args": (stage,)},
    ]
    assert called == expected
    assert_calls(called, expected)
...
@pytest.mark.skip(reason="Not ported to support REAX yet")
def test_trainer_datamodule_hook_system(tmp_path):
    ...
    trainer = reax.Trainer(
        ...
        enable_progress_bar=False,
        enable_model_summary=False,
        # TODO: reload_dataloaders_every_n_epochs=1,
    )

    called = []
    dm = HookedDataModule(called)
    trainer.fit(
        model, datamodule=dm, max_epochs=1, limit_train_batches=batches, limit_val_batches=batches
    )
    expected = [
        {"name": "prepare_data"},
        {"name": "setup", "kwargs": {"stage": "fit"}},
    ...
    called = []
    dm = HookedDataModule(called)
    trainer.validate(model, datamodule=dm, verbose=False, max_batches=1, limit_batches=batches)
    expected = [
        {"name": "prepare_data"},
    ...
    called = []
    dm = HookedDataModule(called)
    trainer.test(model, datamodule=dm, verbose=False, max_batches=1, limit_batches=batches)
    expected = [
        {"name": "prepare_data"},
    ...
    called = []
    dm = HookedDataModule(called)
    trainer.predict(model, datamodule=dm, max_batches=1, limit_batches=batches)
    expected = [
        {"name": "prepare_data"},
...
#     expected_lm_called += [
#         {"name": "on_load_checkpoint", "args": ({**saved_ckpt, "hyper_parameters": ANY},)}
#     ]
#     assert lm_called == expected_lm_called
#     assert ldm_called == [
#         {"name": "load_state_dict", "args": (saved_ckpt[datamodule_state_dict_key],)}
#     ]
#
#
